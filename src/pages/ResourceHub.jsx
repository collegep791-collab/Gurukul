...
export default function ResourceHub() {
  const { resources, addResource, searchQuery, setSearchQuery } = useData();
  ...
  const openResource = (res) => {
    if (res.thumbnail && res.thumbnail.startsWith('http')) {
      ...
      <section className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12">
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          {filter === 'All' && !searchQuery && featured && (
            ...
            >
              <img 
                src={featured.thumbnail || "https://images.unsplash.com/photo-1614728263952-84ea256f9679?auto=format&fit=crop&w=1200&q=80"} 
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-2 border-b border-outline-variant/10 dark:border-slate-800">
            ...
            <div className="flex items-center gap-4">
              <p className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-widest">{filteredResources.length} Assets</p>
            </div>
          </div>
            ...
            {filteredResources.map(res => (
              <div key={res.id} className="bg-surface-container-lowest dark:bg-slate-900 rounded-2xl overflow-hidden border border-outline-variant/5 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group cursor-pointer" onClick={() => openResource(res)}>
                <div className="aspect-video relative bg-surface-container-low dark:bg-slate-800 overflow-hidden">
                  <img 
                    src={res.thumbnail || "https://images.unsplash.com/photo-1614728263952-84ea256f9679?auto=format&fit=crop&w=600&q=80"} 
                ...
                <div className="p-5">
                  ...
                  <h4 className="font-black text-on-surface dark:text-white mb-4 line-clamp-2 min-h-[40px] text-sm">{res.title}</h4>
        ...
        <aside className="space-y-8">
          <div className="bg-surface-container-low dark:bg-slate-900 p-6 rounded-2xl border border-outline-variant/5 dark:border-slate-800">
            ...
            <div className="space-y-4">
              {[
                { label: 'Computer Science', count: resources.filter(r => r.category === 'Computer Science').length, icon: 'code' },
                { label: 'Physics', count: resources.filter(r => r.category === 'Physics').length, icon: 'science' },
                { label: 'Mathematics', count: resources.filter(r => r.category === 'Mathematics').length, icon: 'calculate' },
                { label: 'All Resources', count: resources.length, icon: 'library_books' },
              ].map(item => (
                <button key={item.label} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-white dark:hover:bg-slate-800 transition-all group">
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-xl text-outline dark:text-slate-500 group-hover:text-primary dark:group-hover:text-indigo-400">{item.icon}</span>
                    <span className="text-sm font-bold text-on-surface-variant dark:text-slate-400 group-hover:text-on-surface dark:group-hover:text-white">{item.label}</span>
                  </div>
                  <span className="bg-white dark:bg-slate-800 px-2 py-0.5 rounded-md text-[10px] font-black text-primary dark:text-indigo-400 shadow-sm">{item.count}</span>
                </button>
              ))}
            </div>
          </div>
