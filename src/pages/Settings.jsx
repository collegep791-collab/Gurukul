...
export default function Settings() {
  ...
  const handleSaveProfile = async () => {
    setProfileMsg('');
    try {
      const updated = await api.patch('/users/me', { name, email, usn, class: className, section });
    ...
    <DashboardLayout>
      <div className="px-6 py-4">
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          {/* Sidebar */}
          <aside className="w-full lg:w-72 flex flex-col gap-8 flex-shrink-0">
            <nav className="flex flex-row lg:flex-col gap-2 bg-surface-container-low dark:bg-slate-900 p-2 rounded-2xl lg:rounded-[32px] border border-outline-variant/5 dark:border-slate-800 shadow-sm overflow-x-auto lg:overflow-x-visible scrollbar-hide">
              {tabs.map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-4 px-5 lg:px-6 py-3 lg:py-4 text-xs lg:text-sm font-black rounded-xl lg:rounded-2xl transition-all uppercase tracking-widest whitespace-nowrap ${activeTab === tab.id ? 'bg-white dark:bg-slate-800 text-primary shadow-xl' : 'text-outline dark:text-slate-400 hover:text-on-surface dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-800/50'}`}
                >
              ...
              <section className="bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant/10 dark:border-slate-800 rounded-[40px] p-10 shadow-xl space-y-8">
                ...
                <div className="flex items-center gap-10 p-8 bg-surface-container-low/40 dark:bg-slate-800/40 rounded-[32px]">
                  <div className="relative group">
                    <img className="w-28 h-28 rounded-[28px] object-cover ring-8 ring-white dark:ring-slate-900 shadow-2xl" src={user.avatar || `https://api.dicebear.com/9.x/notionists/svg?seed=${user.name}`} alt={user.name} />
                    <input
                      type="file"
                      ref={fileInputRef}
                      className="hidden"
                    ...
                    <button 
                      ...
                      className="absolute -bottom-2 -right-2 bg-primary text-white h-10 w-10 rounded-xl border-4 border-white dark:border-slate-900 shadow-xl flex items-center justify-center hover:scale-110 transition-all"
                    >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px]">
                      {user.role} {user.usn ? `• ${user.usn}` : ''} • Joined {user.date_joined ? new Date(user.date_joined).toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px] ml-1">Full Name</label>
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px] ml-1">Institutional Email</label>
                  ...
                  {user.role === 'STUDENT' && (
                    <>
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px] ml-1">USN (1RL24SCSXX)</label>
                        <input value={usn} onChange={e => setUsn(e.target.value)} className="w-full px-6 py-4 bg-surface-container-low dark:bg-slate-800 border border-transparent rounded-2xl text-sm font-black text-on-surface dark:text-white focus:border-primary outline-none transition-all" type="text" />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px] ml-1">Class</label>
                          <select value={className} onChange={e => setClassName(e.target.value)} className="w-full px-6 py-4 bg-surface-container-low dark:bg-slate-800 border border-transparent rounded-2xl text-sm font-black text-on-surface dark:text-white focus:border-primary outline-none transition-all appearance-none">
                            {["1st Year", "2nd Year", "3rd Year", "4th Year"].map(c => <option key={c} value={c}>{c}</option>)}
                          </select>
                        </div>
                        <div className="space-y-3">
                          <label className="text-[10px] font-black text-outline dark:text-slate-500 uppercase tracking-[2px] ml-1">Section</label>
                          <select value={section} onChange={e => setSection(e.target.value)} className="w-full px-6 py-4 bg-surface-container-low dark:bg-slate-800 border border-transparent rounded-2xl text-sm font-black text-on-surface dark:text-white focus:border-primary outline-none transition-all appearance-none">
